...
|------------|------------|------|-----------------------------------|-------------------------------------------------|
| 2026-07-04 | this commit | [CX] | BUILD VERIFY — current rally/dev  | xcodebuild generic iOS Simulator Debug **BUILD SUCCEEDED** at `2bc4dcb`; no code changes. Confirms the real-phone checklist starts from a compiling tree while preserving the existing dirty `agents/handoff.md`. |
| 2026-07-03 | this commit | [CX] | Real phone test checklist         | Added `RALLY_REAL_PHONE_TEST.md` for the next device run: launch quiet defaults, avatar identity, outfit slot isolation, gameplay feed, tennis feel, audio toggle, Shop/Journal/World, final 1-10 scores, and top-three bug capture. Docs-only; build not required. |
