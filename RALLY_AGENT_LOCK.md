...
|------------------|-------|--------------|--------|-------|
| 2026-09-07 | [CX] | Rally Coach additive feature (`Rally/Features/Coach/`), focused tests and package, Home toolbar and Training entry points, Xcode registration, Coach handoff docs | ACTIVE | Owner requested Windows/GitHub preparation for laptop and iPhone implementation. Isolated `codex/rally-coach-ios` from `rally/dev` at 481157f; Mac build/device verification handed to laptop. Existing gameplay, avatar, commerce, sync and local laptop tweaks stay out of scope. |
